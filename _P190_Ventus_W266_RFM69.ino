...
#define PLUGIN_190_DEBUG            true                        // Shows received frames and crc in log@INFO

#define PLUGIN_190
#define PLUGIN_ID_190               190
#define PLUGIN_NAME_190             "Ventus W266 RFM69"
#define PLUGIN_VALUENAME1_190       ""
#define PLUGIN_VALUENAME2_190       ""
#define PLUGIN_VALUENAME3_190       ""

#define Plugin_190_RESET_Pin        4
#define Plugin_190_DIO0_Pin         5
#define Plugin_190_SPI_CS_Pin       15
...
{
  digitalWrite(Plugin_190_SPI_CS_Pin, LOW);
  SPI.transfer(address | 0x80);
  SPI.transfer(data);
  digitalWrite(Plugin_190_SPI_CS_Pin, HIGH);
}
...
void RFM69init()
{  
  digitalWrite(Plugin_190_RESET_Pin, HIGH);
  delay(10);
  digitalWrite(Plugin_190_RESET_Pin, LOW);
  delay(10);
...
boolean Plugin_190(uint8_t function, struct EventStruct *event, String& string)
{
  ...
  switch (function)
  {
    case PLUGIN_DEVICE_ADD:
      {
        Device[++deviceCount].Number = PLUGIN_ID_190;
    ...
    case PLUGIN_GET_DEVICENAME:
      {
        string = F(PLUGIN_NAME_190);
        break;
      }

    case PLUGIN_GET_DEVICEVALUENAMES:
      {
        strcpy_P(ExtraTaskSettings.TaskDeviceValueNames[0], PSTR(PLUGIN_VALUENAME1_190));
        strcpy_P(ExtraTaskSettings.TaskDeviceValueNames[1], PSTR(PLUGIN_VALUENAME2_190));
        strcpy_P(ExtraTaskSettings.TaskDeviceValueNames[2], PSTR(PLUGIN_VALUENAME3_190));        
        break;
      }
    ...
    case PLUGIN_WEBFORM_LOAD:
      {
        ...
        if (instanceType == INSTANCE_TH)
          addFormNumericBox(F("Unit ID"), F("plugin_190_unitID"), Settings.TaskDevicePluginConfig[event->TaskIndex][1], 0, 255);
          
        addFormSelector(F("Plugin function"), F("plugin_190_instanceType"), NUMBER_OF_INSTANCES, instance, NULL, instanceType);
    ...
    case PLUGIN_WEBFORM_SAVE:
      {
        Settings.TaskDevicePluginConfig[event->TaskIndex][0] = getFormItemInt(F("plugin_190_instanceType"));
        Settings.TaskDevicePluginConfig[event->TaskIndex][1] = getFormItemInt(F("plugin_190_unitID"));
        success = true;
        break;
      }
    ...
    case PLUGIN_INIT:
      {
        ...
        switch (Settings.TaskDevicePluginConfig[event->TaskIndex][0])
        {
          case INSTANCE_TH:
            {
              pinMode(Plugin_190_RESET_Pin, OUTPUT);
              pinMode(Plugin_190_DIO0_Pin, INPUT);
              pinMode(Plugin_190_SPI_CS_Pin, OUTPUT);

              // initialize SPI
              SPI.setHwCs(false);
              SPI.begin();
              addLog(LOG_LEVEL_INFO, F("P190 : SPI Init"));

              // initilize RFM69
              RFM69init();
              
              addLog(LOG_LEVEL_INFO, F("P190 : RFM69 Init"));
    ...
    case PLUGIN_ONCE_A_SECOND:
      {
        ...
        if (Settings.TaskDevicePluginConfig[event->TaskIndex][0] == INSTANCE_TH)        
        {
          ...
          else
          {
            ...
            if (digitalRead(Plugin_190_DIO0_Pin) == HIGH)
            {
              uint8_t i = 0;
              ...
              while ((digitalRead(Plugin_190_DIO0_Pin) == HIGH) && (i < PAYLOAD_SIZE))
                buffer[i++] = SPI.transfer(0x00);
              digitalWrite(Plugin_190_SPI_CS_Pin, HIGH);
      
#ifdef PLUGIN_190_DEBUG
              // CRC correct?
              if (buffer[21] == calcCRC((uint8_t*)buffer, (PAYLOAD_SIZE - 1)))
              {
                String log = F("P190 : RX: ");
